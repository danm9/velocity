...
def velocity_calc(): 
    ...
    Plan For {round(plan_for)}, Total for the Sprint Would Be {round(plan_for) + round(injection_points) + round(rollover_points)}
            ...
            try:
                ...
                old_team_count = float(input("Team Members in Previous Sprint: ")) 
            try:
                sprint_length = float(input("How Long is Your Sprint (1-4 Weeks)? "))           
            except ValueError:
                print("Not a Valid number")
                sprint_length = float(input("How Long is Your Sprint (1-4 Weeks)? ")) 
            try:
            ...
            sprint_day_count = sprint(sprint_length)
            determine_velocity = float(current_velocity) / (float(old_team_count) * float(sprint_day_count))
            new_velocity = float(determine_velocity) * float(new_team_members) * sprint_day_count
